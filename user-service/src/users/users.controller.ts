...
export class UsersController {
  ...
  async create(@Body() createUserDto: CreateUserDto) {
    ...
    let serviceResult = await this.usersService.create(
      name,
      email,
      password,
      age,
    );
    return { users: serviceResult, description: `the mentioned user added` };
  }

  @GrpcMethod('UserService', 'findAll')
  async findAll() {
    let serviceResult = await this.usersService.findAll();
    return { users: serviceResult, description: 'all mentioned users got' };
  }
  ...
  async findOne(@Body() getUserDto: GetUserDto) {
    ...
    if (serviceResult)
      return { users: serviceResult, description: 'the mentioned user got' };
  ...
  async update(@Body() updateUserDto: UpdateUserDto) {
    ...
    if (serviceResult.affected)
      return {
        users: serviceResult,
        description: 'the mentioned user updated',
      };
    throw new GrpcNotFoundException('user not found');
  }
  ...
  async remove(@Body() removeUserDto: RemoveUserDto) {
    ...
    if (serviceResult.affected)
      return {
        users: serviceResult,
        description: 'the mentioned user deleted',
      };
    throw new GrpcNotFoundException('user not found');
  }
}
